...
두 파일 모두 한 줄에 한 레코드를 둔 JSON Lines다. 사람이 열어 읽고 고치기 쉽다. 버린 기록 비우기는 결과가 버림인 줄을 지우는 것이다. 남김 줄은 지우지 않는다. 실행 요약의 줄마다 그때 처리 이력에 남은 남김 줄 수가 적혀 있어, 이 수가 줄면 다음 실행이 판별 전에 멈춘다([[CCR-UC-001#UC-S4]] 2c). 예전 판으로 되돌릴 때는 두 파일을 같은 커밋의 판으로 함께 되돌린다([[CCR-UC-001#UC-A3]] 2d2).
...
- **판별**: 키가 틀렸거나(401), 한도·크레딧이 소진됐거나(429 가운데 지출 한도·크레딧 소진), 설정한 모델이 없다는(404) 응답은 다시 시도하지 않는다. 남은 묶음도 묻지 않고 판별 실패로 센다. 어느 묶음을 물어도 같은 답이 오기 때문이다([[CCR-UC-001#UC-S5]] 2a). 첫 호출부터 이 응답이 오면 판별 실패가 절반을 넘어 미룸이 되고, 경고의 원인은 호출 실패로 남는다([[CCR-UC-001#UC-S5]] 2c). 한도처럼 도중에 닿아 실패가 절반 이하면, 실패한 묶음은 남김으로 적재된다([[CCR-UC-001#UC-S5]] 2b). 일시적인 속도 제한(429)만 8.5대로 다시 시도한다.
...
4. 배치가 쓴 실행 요약 줄을 3과 같이 확인해 붙인다. 그 줄이 없으면 배치가 줄을 남기기 전에 멈춘 것이므로, 결과 **중단**의 줄을 만들어 붙이고 이 스텝을 실패로 끝내기로 표시한다. 이 줄에는 `RUN_STARTED_AT`에서 구한 기준일, 실행 식별자, 실행 종류(`GITHUB_EVENT_NAME`이 `schedule`이면 예약, 그 밖은 수동)와 남김 줄 수만 적는다. 소스별 건수·탈락 건수·경고는 0으로 채우지 않고 비워 둔다. 0건 경고 판단([[CCR-UC-001#UC-S7]] 2)이 이 줄을 그 소스를 수집하지 않은 줄로 보고 건너뛰게 하기 위해서다. 배치가 쓴 줄이든 중단 줄이든, 남김 줄 수는 3에서 추가분을 얹은 뒤의 처리 이력에서 세어 채운다([[CCR-UC-001#UC-S4]] 2c). 배치가 미리 세면 3에서 붙이지 못한 줄만큼 실제보다 커져, 다음 실행이 멀쩡한 처리 이력을 줄었다고 보고 멈춘다.
...
|---|---|---|---|---|
| 대회 소스 여섯 | 20초 | 연결 오류·타임아웃·429·5xx·형식이 예상과 다른 응답에 2회. 2초, 4초로 늘려 가며. `Retry-After`가 오면 그만큼 기다리되, 30초나 그 소스의 남은 예산보다 길면 기다리지 않고 끝내 실패한 것으로 본다 | 여섯 소스를 동시에 돌린다. 같은 소스 안에서는 요청 사이 1초. 소스별 쪽 상한 20. 소스별 시간 예산 2분 | 빈 목록과 실패 표시([[CCR-UC-001#UC-S1]] 2a). 예산을 넘긴 소스도 같다([[CCR-UC-001#UC-S1]] \*a) |
...
행·컬럼 만들기를 그대로 다시 보내지 않는 것은 노션에 같은 행이 둘 생기지 않게 하기 위해서다([[CCR-UC-001#UC-S6]] 2a1). 노션 공식 문서도 멱등이 아닌 쓰기는 429와 529만 다시 보내라고 한다. 503이 새 행의 ID를 알려 주면 행이 만들어진 것으로 보고 곧바로 처리 이력에 적는다([[CCR-UC-001#UC-S6]] 2d). 행이 생겼는데 응답만 받지 못한 대회는 다음 실행이 노션 현재 행으로 알아보므로 중복되지 않는다([[CCR-UC-001#UC-S6]] 2c).
...
| 처리 이력의 남김 줄이 줄어듦(잘못된 손질·처리 이력만 되돌림) | 실패(처리 이력 감소) | 실패 | 판별도 적재도 하지 않는다. 저장소 이력에서 지워진 줄을 되살린다. 일부러 되돌린 것이면 실행 요약도 같은 판으로 되돌린다 ([[CCR-UC-001#UC-A3]] 2i) |
| 처리 이력과 실행 요약이 함께 사라짐(폴더 이동 등) | 성공 | 성공 | 견줄 값이 없어 첫 실행처럼 돈다. 지운 대회가 되살아나고 버린 대회를 다시 판별한다. 받아들인 한계다 ([[CCR-UC-001#UC-S4]] 2a) |
...
| 수동 취소 | 중단(마무리 단계가 씀) | 취소 | 마무리 단계가 실패로 끝나도 잡은 취소로 남아 실패 알림이 오지 않는다. 오늘 마감 대회가 남았을 수 있으면 그날 안에 다시 돌린다 ([[CCR-UC-001#UC-A1]] \*a2) |
| 배치가 뜨기 전 실패(코드 받기·정의와 코드 어긋남·uv 설치) | 중단(마무리 단계가 씀) | 실패 | 정의가 바뀐 것이면 새 수동 실행으로 돌린다 (8.1 · [[CCR-UC-001#UC-A1]] 1c4) |
| 쓰기 여부 값이 `true`·`false`가 아님(워크플로 수정 실수) | 줄 없음 | 실패 | 배치도 마무리 단계도 쓰지 않는다. `daily.yml`을 고친다 |
| 마무리 단계가 돌지 못하거나 끊김(강제 취소·잡 시간 한도·러너 유실) | 줄 없음 | 실패 또는 취소 | 노션에 행이 들어갔을 수 있다. 로그의 적재 목록을 보고 그날 새 수동 실행을 돌린다 ([[CCR-UC-001#UC-A1]] \*a5 · [[CCR-UC-001#UC-A3]] 1a3) |
